...
public class ArrayDeque<T> extends AbstractDeque<T> {
    ...
    public T removeLast() {
        ...
        size -= 1;
        if (needsDownsize()) {
            System.out.println("error occuring " + size + " " + data.length);
            resize(data.length / 2);
        }
        return result;
    }
    ...
    @SuppressWarnings("unchecked")
    private void resize(int capacity) { // capacity = 8
        T[] newData = (T[]) new Object[capacity]; //capacity = 8
        System.out.println(front + " " + size);
        int i = increment(front, size);
        for (int newIndex = 0; newIndex < size; newIndex += 1) {
            System.out.println(newIndex + " " + size + " " + i);
            if (i >= data.length) {
                i = 0;
            }
